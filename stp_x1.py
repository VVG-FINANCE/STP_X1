...
class TradingEngine:
    def start_trade(self,account,amount,signal,sl,tp1,tp2,tp3,safety_zone,wave_threshold,real=False):
        trade = {"amount":amount,"signal":signal,"sl":sl,"tp1":tp1,"tp2":tp2,"tp3":tp3,
                 "safety_zone":safety_zone,"wave_threshold":wave_threshold,"real":real,
                 "timestamp":datetime.now(),"status":"Ativo"}
...
# =========================
    ...
    fig = go.Figure()
    fig.add_trace(go.Candlestick(x=market_data["Date"], open=market_data["Open"],
                                 high=market_data["High"], low=market_data["Low"],
                                 close=market_data["Close"], name="Candles"))
...
# =========================
    ...
    relatorio = {"Período":periodo,"Data":datetime.today().strftime("%Y-%m-%d"),
